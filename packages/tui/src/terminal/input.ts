...
/** Shared arrow-key decoder for both CSI and SS3 sequences. */
function arrowKey(c: string): KeyName | null {
  switch (c) {
    case 'A':
      return 'up';
    case 'B':
...
function parseCsi(s: string): EscapeParse | null {
  ...
  const arrow = arrowKey(final);
  if (arrow) return { events: [{ type: 'key', name: arrow }], consumed };
